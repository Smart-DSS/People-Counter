...
while True:
    ...
    # Draw counting lines
    cv2.line(frame, (37, cy1), (845, cy1), (0, 255, 0), 2, 2)
    cv2.line(frame, (127, cy2), (825, cy2), (0, 255, 255), 2, 2)
    ...
    timestamp_utc = datetime.utcnow()  # Get current UTC time
    timestamp_ist = timestamp_utc.replace(tzinfo=pytz.utc).astimezone(ist)  # Convert to IST
    ...
    if current_time - last_hourly_store_time >= hourly_interval_duration:
        ...
        last_hourly_store_time = current_time  # Update last hourly storage time
    ...
    if total > 3:
        if current_time - last_exceed_store_time >= exceed_interval_duration:  # Check if one minute has passed
            ...
            last_exceed_store_time = current_time  # Update last exceed data storage time
